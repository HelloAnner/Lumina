...
test("extractPdfMetadataFromDocument 会修正大写标题中的断词空格", async () => {
  const result = await extractPdfMetadataFromDocument(
    {
      numPages: 1,
      async getMetadata() {
        return { info: { Title: "Heading Word Merge" } }
      },
      async getOutline() {
        return []
      },
      ...
      async getPage() {
        return {
          async getTextContent() {
            return {
              items: [
                { str: "REAC T", hasEOL: false },
                { str: " ", hasEOL: false },
                { str: "MODEL", hasEOL: false },
                { str: " ", hasEOL: false },
                { str: "BEHAVIOR", hasEOL: true }
              ]
            }
          }
        }
    ...
    "heading-word-merge.pdf"
  )

  assert.match(result.sections[0].content, /REACT MODEL BEHAVIOR/)
})

test("extractPdfMetadataFromDocument 会把脚注编号开头的行保留为新段落", async () => {
  const result = await extractPdfMetadataFromDocument(
    {
      numPages: 1,
      async getMetadata() {
        return { info: { Title: "Footnote Paragraphs" } }
      },
      async getOutline() {
        return []
      },
      ...
      async getPage() {
        return {
          async getTextContent() {
            return {
              items: [
                { str: "Author A, Author B", hasEOL: true },
                { str: "1Department of Computer Science", hasEOL: true },
                { str: "2Research Lab", hasEOL: true }
              ]
            }
          }
        }

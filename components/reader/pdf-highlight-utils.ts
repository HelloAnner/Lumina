...
interface NormalizePdfDragRectInput {
  pageRect: PdfHighlightRect
...
export function normalizePdfDragRect({
  pageRect,
  dragRect
}: NormalizePdfDragRectInput) {
  const intersected = intersectRect(dragRect, pageRect)
  if (!intersected) {
    return null
  }

  return {
    left: roundRectValue(intersected.left - pageRect.left),
    top: roundRectValue(intersected.top - pageRect.top),
    width: roundRectValue(intersected.width),
    height: roundRectValue(intersected.height)
  }
}

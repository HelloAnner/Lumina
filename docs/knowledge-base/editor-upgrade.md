...
当前编辑器已经基于 TipTap 2.x 落地，具备基础富文本、块菜单、拖拽手柄、快捷键和自定义块节点。

当前主要问题不再是“缺少编辑能力”，而是：

- 输入链路仍存在整份 `NoteBlock[]` 快照同步
- 页面容器仍保留旧编辑器时代的状态与保存残留
- 块操作和保存链路缺少局部更新 discipline
- 目录、gutter、右侧对话仍会被编辑热路径放大

性能与体验的主要目标，已经从“补能力”切换为“给现有能力降温”。

| 文件 | 职责 |
|------|------|
| `note-editor.tsx` | TipTap 编辑器主体、块交互、快捷键、保存调度 |
| `note-editor-doc.ts` | `NoteBlock[]` 与 TipTap 文档互转、文档级块操作 |
| `note-editor-nodes.tsx` | 自定义块节点与 NodeView |
| `knowledge-client.tsx` | 页面容器、观点切换、右侧面板协调 |

### 1.2 差距总览

| 能力 | Notion | 当前 | 优先级 |
|------|--------|------|--------|
| 输入响应与滚动连续性 | 极稳 | 中大型笔记仍有粘滞 | P0 |
| 事务级局部更新 | 完整 | 仍有整份快照同步与整篇替换 | P0 |
| 保存链路无感知 | 本地事务 + 异步持久化 | 仍是节流后的全量保存 | P0 |
| 页面级隔离 | 编辑器内核独立 | 页面容器仍承担过多协调 | P0 |
| 侧边能力旁路化 | 编辑不受 chat/annotation 干扰 | 仍有一定耦合 | P1 |
| DOM 读写节制 | decoration / observer 优先 | 仍存在 hover/outline 测量成本 | P1 |
